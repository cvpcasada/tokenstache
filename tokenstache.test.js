...
describe(`tokenize()`, () => {
  ...
  it(`should tokenize with handlebar using view object`, () => {
    expect(tokenize(`hello world {{user}}`, { user: `jimmy` })).toEqual([
      `hello`,
      `world`,
      `jimmy`
    ]);
  ...
  it(`should not include handlebars that doesnt resolve with view object`, () => {
    expect(tokenize(`hello world {{notUser}}`, { user: `jimmy` })).toEqual([
      `hello`,
      `world`
    ]);
  });
  ...
  it(`can resolve view as function`, () => {
    expect(tokenize(`hello world {{user}}`, () => `jimmy`)).toEqual([
      `hello`,
      `world`,
      `jimmy`
    ]);
